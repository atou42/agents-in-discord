...
The lock also enforces asset and evidence coverage: `keyCharacterAssets` must be at least `tier1Characters` (or `--waive-asset-coverage` with a documented reason), and `full_world` locks require raw source outputs in `coverage/observed_surface_evidence/`. The cover gate additionally fails if any live character atom has no archived asset whose filename matches its name, so a cast cannot be half-illustrated behind two green numbers. The Fandom reference-pack floor scales with `tier1Characters` for large `full_world` runs. `full_world` style decisions fail without a recorded `userApproval` (approvedBy user, ISO timestamp, at least two candidatesPresented), because art direction is an aesthetic judgment the user must confirm. Style and final-acceptance audit files fail without a substantive summary (80+ chars) and a `verifierSessionId`.

Execution is gated on human review of the IP itself: from `world_diagnosis` onward, every stage fails without an approved `ip_proposal.md` (main cast and count, cross-source-verified protagonist/location references, fan red lines, delight plan, 3-5 style candidates with exemplar images) and a `checks/ip_proposal_approval.json` recording the user's decisions and postdating the proposal. This is the phase-one exit door — no import, generation or board work happens on unvalidated IP assumptions.

Platform caps get one documented escape hatch, not a loophole: if the platform hard-rejects atoms of a type (e.g. max 50 characters), the run may represent overflow tier-1 characters as `people`/`place` atoms only via `checks/import_cap_resolution.json` — which must quote the platform rejection verbatim, cover at most 20% of tier1Characters, and carry `userApproval` (approvedBy: user). Import atoms must otherwise use the approved type whitelist; wild types fail the import gate. `final_acceptance` additionally requires a factuality audit (`checks/factuality_audit.json`): a clean-context verifier samples delivered cards and verifies their claims against the source, with any INACCURATE finding blocking acceptance. The gate script itself is hash-locked (`script_integrity.json`); run agents cannot modify gates, and changes require explicit user re-acceptance via `accept-script-hash`.
