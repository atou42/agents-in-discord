...
- `CHILD_THREAD_WORKSPACE_MODE`: child thread workspace strategy; `inherit` (default) reuses the parent channel's explicit workspace, while `separate` makes each child thread use its own provider default or `WORKSPACE_ROOT/<threadId>` fallback
