...
const PROVIDER_RUNTIME_SURFACES = Object.freeze({
  mirasim: Object.freeze({
    ...
    runtimeSummary: { en: 'Mirasim desktop Claude engine, workspace-bound sessions, native login and permissions', zh: 'Mirasim 桌面 Claude 引擎、工作目录绑定会话、原生登录与权限' },
    sessionStore: { en: 'Mirasim desktop session store', zh: 'Mirasim 桌面会话存储' },
    resumeSurface: { en: 'claude:<id> session key; workspace changes reset the binding', zh: '通过 claude:<id> 会话键续接；工作目录变化时重置绑定' },
