  ...
  function formatHelpReport(session) {
    ...
    if (language === 'en') {
      return [
        ...
        provider === 'codex' ? `• \`${slashRef('fork')} [name]\` / \`!fork [name]\` — create a native Codex fork in a new Discord thread` : null,
    ...
    return [
      ...
      provider === 'codex' ? `• \`${slashRef('fork')} [name]\` / \`!fork [name]\` — 用 Codex 原生 fork 创建新 Discord thread` : null,
