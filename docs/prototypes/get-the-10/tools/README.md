...
第一步用 ImageTool 生成三张九宫格。每张图必须是 3x3 网格，九个格子要明显不同。建议每张九宫格对应一个大的探索方向，三张之间差异更大，单张内部差异也要拉开。
...
输出会包含 27 张切片和 `manifest.json`。原型或后续 agent 可以读取这个 manifest，把真实切片放进第一轮候选。
